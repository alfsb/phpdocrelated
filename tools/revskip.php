...
if ( $argc != 2 )
{
  ...
  and print statistics. This script ingnores any commit marked
  with [skip-revcheck], and change output format.
...
$skipHash = []; // Commits that should never be referenced
...
function computeSyncStatus( $enFiles , $trFiles , $gitData , $lang )
{
    ...
    foreach( $enFiles as $filename => $enFile )
    {
        ...
        {
            fwrite( STDERR , "Warn: No hash for en/$filename" . PHP_EOL );
            continue;
        }
